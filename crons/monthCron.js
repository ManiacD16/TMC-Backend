...
cron.schedule("0 0 1 * *", async () => { // Run this cron job on the 1st day of every month
  ...
  try {
    ...
    for (const user of users) {
      ...
      const checkAndAddMonthlyReward = (tierStartDate, rewardAmount, flag) => {
        const monthDifference = Math.floor((currentTime - tierStartDate) / (30 * 24 * 60 * 60 * 1000)); // Calculate month difference
        if (monthDifference >= 1 && !user[flag]) { // If at least one month has passed since the start date
      ...
      if (user.isForMonthlyTier1) checkAndAddMonthlyReward(user.tier1StartDate, 1000, "isForTmcSmartRewardClaimed");
      if (user.isForMonthlyTier2) checkAndAddMonthlyReward(user.tier2StartDate, 5000, "isForTmcRoyalRewardClaimed");
      if (user.isForMonthlyTier3) checkAndAddMonthlyReward(user.tier3StartDate, 10000, "isForTmcChiefRewardClaimed");
      if (user.isForMonthlyTier4) checkAndAddMonthlyReward(user.tier4StartDate, 30000, "isForTmcAmbassadorRewardClaimed");

      // If updates are made, save the user data
      if (Object.keys(updateFields).length > 0) {
        await User.findByIdAndUpdate(user._id, { $set: updateFields }, { new: true });
      }
    }
